/*
 WARNING: THIS WILL NOT COMPILE, IT IS TOTALLY INCOMPLETE!
...
#define ACTION_MOD_PC	0
#define ACTION_MOD_OP	1
#define ACTION_MOD_SPECIAL	2
#define ACTION_MOD_REGIMM	3
#define ACTION_MOD_COP0	4
#define ACTION_MOD_COP1	5
#define ACTION_MOD_TLB	6
#define ACTION_MOD_COP1_BC	7
#define ACTION_MOD_RS	8
#define ACTION_MOD_RT	9
#define ACTION_MOD_RD	10
#define ACTION_MOD_SA	11
#define ACTION_MOD_BASE	12
#define ACTION_MOD_IMM	13
#define ACTION_MOD_OFFSET	14
#define ACTION_MOD_TARGET	15
#define ACTION_MOD_FT	16
#define ACTION_MOD_FS	17
#define ACTION_MOD_FD	18
...
extern void _start (u32 a0) __attribute__ ((section (".main")));
...
const char * action_str[19] = {
"PC",		"Main op",	"Special op",	"Regimm op",
"cop0 op",	"cop1 op",	"tlb op",	"cop1 bc",
"rs",		"rt",		"rd",		"sa",
"base",		"immediate",	"offset",	"target",
"ft",		"fs",		"fd"
};
...
void MFC1(u32 word)
{		//00 (00)
    SetTextString(DisplayList, "mfc1\t%s, %s\n", gpr_rn[getRT(word)], fpr_rn[getFS(word)]);
}
...
void BEQ(u32 word)
{		//04(04)
    SetTextString(DisplayList, "beq\t%s, %s, 0x%08X\n", gpr_rn[getRS(word)], gpr_rn[getRT(word)], getOFFSET(word) + pc);
}
...
void _start(u32 a0)
{
    ...
    default:
        ...
        case BUTTON_DPAD_U:
            ...
            {
            case ACTION_MOD_OP:
                if ((opcode&0xFC000000) < 0xFC000000)
                    opcode += 0x04000000;
                break;
            case ACTION_MOD_RS:
                if ((opcode&0x03E00000) < 0x03E00000)
                    opcode += 0x00200000;
                break;
            case ACTION_MOD_RT:
                if ((opcode&0x001F0000) < 0x001F0000)
                    opcode += 0x00010000;
                break;
            /* Unused in main op types
            case ACTION_MOD_RD:
                if ((opcode&0x0000F800) < 0x0000F800)
                    opcode += 0x00000800;
                break;
            */
            case ACTION_MOD_IMM:
            case ACTION_MOD_OFFSET: /* inb4 signage, gtfo */
                if ((opcode&0x0000FFFF)+inc <= 0x0000FFFF)
                    opcode+=inc;
                break;
            case ACTION_MOD_TARGET:
                if ((opcode&0x03FFFFFF)+inc <= 0x03FFFFFF)
                    opcode+=inc;
                break;
            }
            break;
        case BUTTON_DPAD_D:
            ...
            {
            case ACTION_MOD_OP:
                if ((opcode&0xFC000000) > 0x04000000)
                    opcode -= 0x04000000;
                break;
            case ACTION_MOD_RS:
                if ((opcode&0x03E00000) > 0x00200000)
                    opcode -= 0x00200000;
                break;
            case ACTION_MOD_RT:
                if ((opcode&0x001F0000) > 0x00010000)
                    opcode -= 0x00010000;
                break;
            case ACTION_MOD_IMM:
                if ((opcode&0x0000FFFF)-inc >= 0x00000000)
                    opcode-=inc;
                break;
            case ACTION_MOD_TARGET:
                if ((opcode&0x03FFFFFF)-inc >= 0x00000000)
                    opcode-=inc;
                break;
            }
            break;
        case BUTTON_DPAD_R:
            if (op==J||op==JAL)
            {
                if (action == ACTION_MOD_TARGET)
                    action = ACTION_MOD_OP;
                else
                    action = ACTION_MOD_TARGET;
                break;
            }
            switch(action)
            {
            case ACTION_MOD_PC:
                ...
                break;
            case ACTION_MOD_OP:
                action=ACTION_MOD_RS;
                break;
            case ACTION_MOD_RS:
                action=ACTION_MOD_RT;
                break;
            case ACTION_MOD_RT:
                action=ACTION_MOD_IMM;
                break;
            case ACTION_MOD_IMM:
                action=ACTION_MOD_PC;
                break;
            }
